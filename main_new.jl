...
@views function acoustic2D()
    ...
    dt        = min(dx_gas,dy_gas)/sqrt(k_gas/ρ_gas)/4.1                        # timestep size [s]
    ...
    for iter = 1:nt
        ...
        if mod(iter,nout)==0
            ...
            xlims=(X[1],X[end]), ylims=(Y[1],Y[end]), c=:viridis,xlabel = "x [m]", ylabel = "y [m]",title="Pressure (t=)",dpi=300); frame(anim)
        end
